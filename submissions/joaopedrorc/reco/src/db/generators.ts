...
export function generateCreators(
  totalNumberOfCreators: number,
): ICreatorSeed[] {
  ...
  for (let i = 0; i < totalNumberOfCreators; i++) {
    creators.push({
      ...
      reliabilityScore: parseFloat(
        faker.number.float({ min: 0.7, max: 1.0 }).toFixed(2),
      ),
    });
  }
...
export function generateCampaigns(
  ...
  for (let i = 0; i < totalNumberOfCampaigns; i++) {
    campaigns.push({
      ...
      tagsRequired: faker.helpers.arrayElements(
        [
          'fintech',
          'investimentos',
          'skincare',
...
export function generatePastDeals(
  creators: ICreatorSeed[],
  campaigns: ICampaignSeed[],
): IPastDealSeed[] {
